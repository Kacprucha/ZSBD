...
``
``--
`--` Name: longtext; Type: TABLE; Schema: game_data; Owner: -
``--
``
...
``
``--
`--` Name: longtext_class; Type: TABLE; Schema: game_data; Owner: -
``--
``
...
``CASE
            `WHEN` (q.name IS NOT NULL) THEN ('Nagroda z zadania: '
            `ELSE` 'Loot / Sklep'
        `END` AS item_source
   `FROM` ((((((game_data.iteam_character ic
     ...
     `JOIN` game_data."character" c ON (((ic.character_name)
     `JOIN` game_data.player p ON (((c.player_userame)
...
CREATE VIEW game_data.system_ranking_overview AS
 `SELECT` c.name AS longtext_name,
``'Experience',
...
``--
``
`ALTER` TABLE ONLY game_data.accountstatus ALTER COLUMN statusid SET ;
``
``
``--
...
``--
``
`ALTER` TABLE ONLY game_data.combatlog ALTER COLUMN combatlogid SET ;
``
``
``--
...
``--
``
`ALTER` TABLE ONLY game_data.iteam ALTER COLUMN iteamid SET ;
``
``
``--
...
``--
``
`ALTER` TABLE ONLY game_data.iteam_character ALTER COLUMN inventoryid SET ;
``
``
``--
...
``--
``
`ALTER` TABLE ONLY game_data.itemtype ALTER COLUMN typeid SET ;
``
``
``--
...
``--
``
`ALTER` TABLE ONLY game_data.location ALTER COLUMN locationid SET ;
``
``
``--
...
``--
``
`ALTER` TABLE ONLY game_data.locationtype ALTER COLUMN typeid SET ;
``
``
``--
...
``--
``
`ALTER` TABLE ONLY game_data.loglevel ALTER COLUMN loglevelid SET ;
``
``
``--
...
``--
``
`ALTER` TABLE ONLY game_data.memberstatus ALTER COLUMN memberstatusid SET ;
``
``
``--
...
``--
``
`ALTER` TABLE ONLY game_data.monster ALTER COLUMN monsterid SET ;
``
``
``--
...
``--
``
`ALTER` TABLE ONLY game_data.procedureexecutionlog ALTER COLUMN logid SET ;
``
``
``--
...
``--
``
`ALTER` TABLE ONLY game_data.quest ALTER COLUMN questid SET ;
``
``
``--
...
``--
``
`ALTER` TABLE ONLY game_data.questlog ALTER COLUMN questlogid SET ;
``
``
``--
...
``--
``
`ALTER` TABLE ONLY game_data.queststatus ALTER COLUMN statusid SET ;
``
``
``--
...
``--
``
`ALTER` TABLE ONLY game_data.rarity ALTER COLUMN rarityid SET ;
``
``
``--
...
``--
``
`ALTER` TABLE ONLY game_data.skill ALTER COLUMN skillid SET ;
``
``
``--
...
`11	11	252	rpg_game_db	rpg_admin	` CALL game_data.sp_RunWeeklyMaintenance(); 	failed	ERROR:  Statistisc of longtext (HP: 80, Mana: 50) canot be lower then base value for the character class (HP: 100, Mana: 80).\nCONTEXT:  PL/pgSQL function game_data.fn_checkcharacterstats() line 14 at RAISE\nSQL statement "UPDATE game_data.Character SET Gold = Gold + v_reward_amount WHERE Name = v_top_player_1"\nPL/pgSQL function game_data.sp_runweeklymaintenance() line 52 at SQL statement\n	2025-10-25 16:40:00.009822+00	2025-10-25 16:40:00.013843+00
...
``
``--
`--` Data for Name: longtext; Type: TABLE DATA; Schema: game_data; Owner: -
``--
``
...
``
``--
`--` Data for Name: longtext_class; Type: TABLE DATA; Schema: game_data; Owner: -
``--
``
...
``--
``
`COPY` game_data.combatlog (combatlogid, longtext_name, monster_monsterid, location_locationid, victory, dmgtaken, dmgdelt) FROM stdin;
``\.
``
``
``--
...
``--
``
`COPY` game_data.combatlog_archive (combatlogid, longtext_name, monster_monsterid, location_locationid, victory, dmgtaken, dmgdelt) FROM stdin;
...
``--
``
`COPY` game_data.iteam_character (inventoryid, iteam_iteamid, longtext_name) FROM stdin;
``1	1	Aragorn
``2	2	Aragorn
``3	3	Aragorn
``4	3	Legolas
...
``--
``
`COPY` game_data.questlog (questlogid, longtext_name, quest_questid, queststatus_statusid, atemptnumber, starttime, endtime) FROM stdin;
...
``--
``
`COPY` game_data.race_character (race_name, longtext_name) FROM stdin;
``Human	Aragorn
``Elf	Legolas
``Human	Gandalf
``Dwarf	BobTheBard
...
``--
``
`COPY` game_data.skill_character (skill_skillid, longtext_name) FROM stdin;
``1	Aragorn
``6	Aragorn
``7	Aragorn
``18	Aragorn
...
``
``--
`--` Name: longtext_class character_class_pk; Type: CONSTRAINT; Schema: game_data; Owner: -
``--
``
`ALTER` TABLE ONLY game_data.character_class
    ADD CONSTRAINT longtext_class_pk PRIMARY KEY (character_name, class_name);
``
``
``--
`--` Name: longtext character_pkey; Type: CONSTRAINT; Schema: game_data; Owner: -
``--
``
`ALTER` TABLE ONLY game_data."character"
    ADD CONSTRAINT longtext_pkey PRIMARY KEY (name);
``
``
``--
...
`ALTER` TABLE ONLY game_data.race_character
    ADD CONSTRAINT race_character_pk PRIMARY KEY (race_name, longtext_name);
``
``
``--
...
`ALTER` TABLE ONLY game_data.skill_character
    ADD CONSTRAINT skill_character_pk PRIMARY KEY (skill_skillid, longtext_name);
``
``
``--
...
``
``--
`--` Name: longtext trg_characterlevelup; Type: TRIGGER; Schema: game_data; Owner: -
``--
``
...
``
``--
`--` Name: longtext_class character_class_character_name_fkey; Type: FK CONSTRAINT; Schema: game_data; Owner: -
``--
``
`ALTER` TABLE ONLY game_data.character_class
    `ADD` CONSTRAINT longtext_class_character_name_fkey FOREIGN KEY (character_name) REFERENCES game_data."character"(name);
``
``
``--
`--` Name: longtext_class character_class_class_name_fkey; Type: FK CONSTRAINT; Schema: game_data; Owner: -
``--
``
`ALTER` TABLE ONLY game_data.character_class
    `ADD` CONSTRAINT longtext_class_class_name_fkey FOREIGN KEY (class_name) REFERENCES game_data.class(name);
``
``
``--
`--` Name: longtext character_player_userame_fkey; Type: FK CONSTRAINT; Schema: game_data; Owner: -
``--
``
`ALTER` TABLE ONLY game_data."character"
    `ADD` CONSTRAINT longtext_player_userame_fkey FOREIGN KEY (player_userame) REFERENCES game_data.player(username);
``
``
``--
